...
namespace tobor {
	...
	namespace v1_1 {
		...
		namespace svg {
			...
			//using svg_environment = tobor::latest::svggen::xml_root_element;
		...
		class piece_drawer {
			...
			virtual std::unique_ptr<svg::svg_generator>
			   operator()(const world_type& tobor_world, const drawing_style_sheet& dss, const cell_id_type& cell, const std::string& color) = 0;
		...
		class duck_piece_drawer : public piece_drawer<World_Type> {
			...
			public:
			virtual std::unique_ptr<svg::svg_generator>
			   operator()(const world_type& tobor_world, const drawing_style_sheet& dss, const cell_id_type& cell, const std::string& color) override {
		...
		class tobor_graphics {
			...
			inline static std::unique_ptr<svg::svg_path> get_vertical_grid_element(const world_type& tobor_world, const drawing_style_sheet& dss, std::size_t cell_count_offset) {
				...
				auto go_to_right_bottom_left = std::make_shared<svg::svg_path_elements::l<double>>(dss.HALF_GRID_LINE_WIDTH * 2,
				                                                                                   0,
				                                                                                   0,
				                                                                                   dss.CELL_HEIGHT * tobor_world.get_vertical_size() +
			...
			inline static std::unique_ptr<svg::svg_path> get_horizontal_grid_element(const world_type& tobor_world, const drawing_style_sheet& dss, std::size_t cell_count_offset) {
				...
				auto go_to_bottom_right_top = std::make_shared<svg::svg_path_elements::l<double>>(0,
				                                                                                  dss.HALF_GRID_LINE_WIDTH * 2,
				                                                                                  dss.CELL_WIDTH * tobor_world.get_vertical_size() + dss.HALF_GRID_LINE_WIDTH * 2,
				                                                                                  0,
				                                                                                  0,
				                                                                                  -dss.HALF_GRID_LINE_WIDTH * 2);
				auto go_back                = std::make_shared<svg::svg_path_elements::Z>();
			...
			inline static std::unique_ptr<svg::svg_path> get_horizontal_south_wall(const world_type& tobor_world, const drawing_style_sheet& dss, std::size_t x, std::size_t y) {
				...
				auto start_at_upper_left = std::make_shared<svg::svg_path_elements::M<double>>(dss.LEFT_PADDING + dss.CELL_WIDTH * x,
				                                                                               dss.TOP_PADDING - dss.HALF_WALL_LINE_WIDTH +
				                                                                                  dss.CELL_HEIGHT * (tobor_world.get_vertical_size() - y));
			...
			inline static std::unique_ptr<svg::svg_path>
			   fill_whole_cell(const world_type& tobor_world, const drawing_style_sheet& dss, const cell_id_type& cell_id, const std::string& color) {
				...
				auto start_at_north_west = std::make_shared<svg::svg_path_elements::M<double>>(dss.LEFT_PADDING + dss.CELL_WIDTH * cell_id.get_x_coord(tobor_world),
				                                                                               dss.TOP_PADDING + dss.CELL_HEIGHT * (tobor_world.get_vertical_size() -
				                                                                                                                    cell_id.get_y_coord(tobor_world) - 1));
			...
			inline static std::unique_ptr<svg::svg_generator> draw_blocked_cells(const world_type& tobor_world, const drawing_style_sheet& dss) {
				...
				for (cell_size_int raw_id{ 0 }; raw_id < tobor_world.count_cells(); ++raw_id) {
					...
					if (tobor_world.west_wall_by_id(nid) && tobor_world.east_wall_by_id(nid) && tobor_world.south_wall_by_transposed_id(ntid) &&
					    tobor_world.north_wall_by_transposed_id(ntid)) {
			...
			inline static std::string
			   draw_tobor_world(const world_type& tw, const positions_of_pieces_type& pop, const cell_id_type& target_cell, const coloring& c, piece_shape_selection shape) {
				...
				*/
				...
				auto              svg_root        = std::make_unique<::tobor::latest::svggen::xml_root_element>(svg_root_height,
                                                                                        svg_root_width,
                                                                                        std::make_unique<::tobor::latest::svggen::xml_declaration>(),
                                                                                        std::move(svg_body));
			...
			template <class... T>
			inline static std::string draw_tobor_world_with_cell_markers(const tobor::v1_1::dynamic_rectangle_world<T...>& tw, const std::vector<cell_id_type>& markers
			                                                             /*, coloring*/
			) {
