	...
	namespace v1_0 {
		namespace svg {
			...
			class svg_primitive : public svg_generator {
				...
				inline void set(const std::string& primitive_p) { this->primitive = primitive_p; }

				virtual std::string to_std_string() const override { return primitive; }
			};
			...
			namespace svg_path_elements {
				...
				class M : public svg_path_element {
					...
					virtual std::shared_ptr<svg_path_element> clone() const override { return std::make_shared<M>(*this); }
				...
				class Z : public svg_path_element {
					...
					virtual std::string str() const override { return "Z"; }

					virtual std::shared_ptr<svg_path_element> clone() const override { return std::make_shared<Z>(*this); }
				...
				class l : public svg_path_element {
					...
					virtual std::shared_ptr<svg_path_element> clone() const override { return std::make_shared<l>(*this); }
				...
				class c : public svg_path_element {
					...
					virtual std::shared_ptr<svg_path_element> clone() const override { return std::make_shared<c>(*this); }
				...
				class a : public svg_path_element {
					...
					struct step {
						...
						std::string str() const noexcept {
							return std::to_string(rx) + " " + std::to_string(ry) + " " + std::to_string(x_axis_rotation) + " " + std::to_string(static_cast<int>(large_arc_flag)) +
							       " " + std::to_string(static_cast<int>(sweep_flag)) + " " + std::to_string(x) + " " + std::to_string(y);
						}
					...
					template <class... _Rest>
					a(const step& s, _Rest&&... others) : coordinates({ s }) {
						add_coordinates(std::forward<_Rest>(others)...);
					}

					virtual std::string str() const override {
						return std::accumulate(coordinates.cbegin(), coordinates.cend(), std::string("a"), [](const std::string& acc, const step& el) -> std::string {
							return acc + " " + el.str();
						});
					}

					virtual std::shared_ptr<svg_path_element> clone() const override { return std::make_shared<a>(*this); }
			...
			class svg_path : public svg_generator {
				...
				svg_path(const svg_path& other) : other_properties(other.other_properties) {
					std::transform(other.path_elements.cbegin(),
					               other.path_elements.cend(),
					               std::back_inserter(this->path_elements),
					               [](const std::shared_ptr<svg_path_element>& el) { return el->clone(); });
				}
				...
				virtual std::string to_std_string() const override {
					...
					result += std::string(" d=\"") +
					          std::accumulate(path_elements.cbegin(),
					                          path_elements.cend(),
					                          std::string(""),
					                          [](const std::string& acc, const std::shared_ptr<svg_path_element>& el) -> std::string {
						                          return acc.empty() ? el->str() : acc + " " + el->str();
					                          }) +
					          "\"";
					result +=
					   std::accumulate(other_properties.cbegin(), other_properties.cend(), std::string(""), [](const std::string& acc, const map::value_type& el) -> std::string {
				...
				std::string& fill() { return other_properties["fill"]; }
				std::string& stroke() { return other_properties["stroke"]; }
				std::string& stroke_width() { return other_properties["stroke-width"]; }
				std::string& stroke_linecap() { return other_properties["stroke-linecap"]; }
				std::string& stroke_linejoin() { return other_properties["stroke-linejoin"]; }
			};
		...
		template <class... T>
		inline std::unique_ptr<svg::svg_path> get_vertical_grid_element(const tobor::v1_0::legacy_world<T...>& legacy_world,
		                                                                const drawing_style_sheet&             dss,
		                                                                std::size_t                            cell_count_offset) {
			...
			auto start_at_upper_left = std::make_shared<svg::svg_path_elements::M<double>>(dss.LEFT_PADDING - dss.HALF_GRID_LINE_WIDTH + dss.CELL_WIDTH * cell_count_offset,
			                                                                               dss.TOP_PADDING - dss.HALF_GRID_LINE_WIDTH);
			auto go_to_right_bottom_left = std::make_shared<svg::svg_path_elements::l<double>>(dss.HALF_GRID_LINE_WIDTH * 2,
			                                                                                   0,
			                                                                                   0,
			                                                                                   dss.CELL_HEIGHT * legacy_world.get_vertical_size() + dss.HALF_GRID_LINE_WIDTH * 2,
		...
		template <class... T>
		inline std::unique_ptr<svg::svg_path> get_horizontal_grid_element(const tobor::v1_0::legacy_world<T...>& legacy_world,
		                                                                  const drawing_style_sheet&             dss,
		                                                                  std::size_t                            cell_count_offset) {
			auto grid_element = std::make_unique<svg::svg_path>();
			...
			auto start_at_upper_left    = std::make_shared<svg::svg_path_elements::M<double>>(dss.LEFT_PADDING - dss.HALF_GRID_LINE_WIDTH,
                                                                                        dss.TOP_PADDING - dss.HALF_GRID_LINE_WIDTH + dss.CELL_HEIGHT * cell_count_offset);
			auto go_to_bottom_right_top = std::make_shared<svg::svg_path_elements::l<double>>(0,
			                                                                                  dss.HALF_GRID_LINE_WIDTH * 2,
			                                                                                  dss.CELL_WIDTH * legacy_world.get_vertical_size() + dss.HALF_GRID_LINE_WIDTH * 2,
			                                                                                  0,
			                                                                                  0,
			                                                                                  -dss.HALF_GRID_LINE_WIDTH * 2);
			auto go_back                = std::make_shared<svg::svg_path_elements::Z>();
		...
		template <class... T>
		inline std::unique_ptr<svg::svg_path>
		   get_horizontal_south_wall(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss, std::size_t x, std::size_t y) {
			...
			auto start_at_upper_left = std::make_shared<svg::svg_path_elements::M<double>>(dss.LEFT_PADDING + dss.CELL_WIDTH * x,
			                                                                               dss.TOP_PADDING - dss.HALF_WALL_LINE_WIDTH +
			                                                                                  dss.CELL_HEIGHT * (legacy_world.get_vertical_size() - y));

			auto left_semicircle = std::make_shared<svg::svg_path_elements::c<double>>(-dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
			                                                                           0,
			...
			auto go_right = std::make_shared<svg::svg_path_elements::l<double>>(dss.CELL_WIDTH, 0);

			auto right_semicircle = std::make_shared<svg::svg_path_elements::c<double>>(dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
			                                                                            0,
			                                                                            dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
			                                                                            -2 * dss.HALF_WALL_LINE_WIDTH,
			                                                                            0,
		...
		template <class... T>
		inline std::unique_ptr<svg::svg_path>
		   get_vertical_west_wall(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss, std::size_t x, std::size_t y) {
			...
			auto start_at_lower_left = std::make_shared<svg::svg_path_elements::M<double>>(dss.LEFT_PADDING - dss.HALF_WALL_LINE_WIDTH + dss.CELL_WIDTH * x,
			                                                                               dss.TOP_PADDING + dss.CELL_HEIGHT * (legacy_world.get_vertical_size() - y));

			auto bottom_semicircle = std::make_shared<svg::svg_path_elements::c<double>>(0,
			                                                                             dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
			                                                                             2 * dss.HALF_WALL_LINE_WIDTH,
			                                                                             dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
			                                                                             2 * dss.HALF_WALL_LINE_WIDTH,
			...
			auto go_north = std::make_shared<svg::svg_path_elements::l<double>>(0, -dss.CELL_WIDTH);

			auto top_semicircle = std::make_shared<svg::svg_path_elements::c<double>>(0,
			                                                                          -dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
			                                                                          -2 * dss.HALF_WALL_LINE_WIDTH,
			                                                                          -dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
			                                                                          -2 * dss.HALF_WALL_LINE_WIDTH,
		...
		inline std::unique_ptr<svg::svg_path>
		   fill_whole_cell(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss, const cell_id_type& cell_id, const std::string& color) {
			...
			auto start_at_north_west =
			   std::make_shared<svg::svg_path_elements::M<double>>(dss.LEFT_PADDING + dss.CELL_WIDTH * cell_id.get_x_coord(),
			                                                       dss.TOP_PADDING + dss.CELL_HEIGHT * (legacy_world.get_vertical_size() - cell_id.get_y_coord() - 1));

			auto go_east = std::make_shared<svg::svg_path_elements::l<double>>(dss.CELL_WIDTH, 0);

			auto go_south = std::make_shared<svg::svg_path_elements::l<double>>(0, dss.CELL_HEIGHT);

			auto go_west = std::make_shared<svg::svg_path_elements::l<double>>(-dss.CELL_WIDTH, 0);
		...
		template <class... T>
		inline std::unique_ptr<svg::svg_generator> draw_blocked_cells(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss) {
			...
			for (cell_id_int_type cell_id{ 0 }; cell_id < legacy_world.count_cells(); ++cell_id) {
				...
				if (legacy_world.west_wall_by_id(cell_id) && legacy_world.east_wall_by_id(cell_id) &&
					...
					auto block = fill_whole_cell(legacy_world, dss, redundant_cell_id, "black");
		...
		inline std::unique_ptr<svg::svg_generator> draw_walls(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss) {
			...
			for (int_type x = 0; x < legacy_world.get_horizontal_size(); ++x) {
				for (int_type y = 0; y <= legacy_world.get_vertical_size(); ++y) {
					if (legacy_world.south_wall_by_transposed_id(
					       tobor::v1_0::redundant_cell_id<tobor::v1_0::legacy_world<T...>>::create_by_coordinates(x, y, legacy_world).get_transposed_id())) {
			...
			for (int_type x = 0; x <= legacy_world.get_horizontal_size(); ++x) {
				for (int_type y = 0; y < legacy_world.get_vertical_size(); ++y) {
					if (legacy_world.west_wall_by_id(tobor::v1_0::redundant_cell_id<tobor::v1_0::legacy_world<T...>>::create_by_coordinates(x, y, legacy_world).get_id())) {
						// ! Note that y exceeds its natural range by 1. But it is okay, since we operate on an infinite repeating 2D landscape
		...
		class piece_drawer {
			...
			virtual std::unique_ptr<svg::svg_generator> operator()(const world_type&                                 legacy_world,
			                                                       const drawing_style_sheet&                        dss,
		...
		class duck_piece_drawer : public piece_drawer<WorldType> {
			...
			virtual std::unique_ptr<svg::svg_generator> operator()(const world_type&                                 legacy_world,
				...
				auto start_at_south_west = std::make_shared<svg::svg_path_elements::M<double>>(CELL_CORNER_SOUTH_WEST_x + dss.HORIZONTAL_PIECE_PADDING,
				                                                                               CELL_CORNER_SOUTH_WEST_y - dss.VERTICAL_PIECE_PADDING);

				auto go_bottom_east = std::make_shared<svg::svg_path_elements::l<double>>(dss.CELL_WIDTH - 2 * dss.HORIZONTAL_PIECE_PADDING, 0);

				auto go_bottom_right_corner_north = std::make_shared<svg::svg_path_elements::l<double>>(0, -FOOT_THICKNESS_RIGHT);

				auto go_back_neck_bottom_up = std::make_shared<svg::svg_path_elements::c<double>>(-0.5 * CANVAS_X_SIZE,
				                                                                                  0,
				                                                                                  -0.5 * CANVAS_X_SIZE,
				                                                                                  0,
				                                                                                  -0.4 * CANVAS_X_SIZE,
				                                                                                  -(0.45 * CANVAS_Y_SIZE - FOOT_THICKNESS_RIGHT));

				auto go_head = std::make_shared<svg::svg_path_elements::a<double>>(svg::svg_path_elements::a<double>::step(dss.PIECE_HEAD_X_RADIUS_FACTOR * MINIMUM_CANVAS_SIZE,
				                                                                                                           dss.PIECE_HEAD_Y_RADIUS_FACTOR * MINIMUM_CANVAS_SIZE,
				                                                                                                           dss.PIECE_HEAD_AXIS_ROTATION,
				                                                                                                           true,
				                                                                                                           false,
				                                                                                                           -0.2 * MINIMUM_CANVAS_SIZE,
				                                                                                                           -0.25 * MINIMUM_CANVAS_SIZE));

				auto go_nort_west_for_beak = std::make_shared<svg::svg_path_elements::l<double>>(-0.33 * MINIMUM_CANVAS_SIZE, -0.15 * MINIMUM_CANVAS_SIZE);

				auto go_south_east_for_beak_down = std::make_shared<svg::svg_path_elements::l<double>>(0.32 * MINIMUM_CANVAS_SIZE, 0.25 * MINIMUM_CANVAS_SIZE);

				auto go_back = std::make_shared<svg::svg_path_elements::Z>();

				duck->path_elements = { start_at_south_west,         go_bottom_east, go_bottom_right_corner_north, go_back_neck_bottom_up, go_head, go_nort_west_for_beak,
				...
				auto start_eye = std::make_shared<svg::svg_path_elements::M<double>>(CELL_CORNER_SOUTH_WEST_x + dss.HORIZONTAL_PIECE_PADDING + 0.66 * MINIMUM_CANVAS_SIZE,
				                                                                     CELL_CORNER_SOUTH_WEST_y - (dss.VERTICAL_PIECE_PADDING + 0.77 * MINIMUM_CANVAS_SIZE));

				outer_eye->path_elements.push_back(start_eye);

				outer_eye->path_elements.push_back(std::make_shared<svg::svg_path_elements::a<double>>(
		...
		class ball_piece_drawer : public piece_drawer<WorldType> {
			...
			virtual std::unique_ptr<svg::svg_generator> operator()(const world_type&                                 legacy_world,
				...
				marker->path_elements.push_back(std::make_shared<svg::svg_path_elements::M<double>>(CENTER_X, CENTER_Y + HALF_CANVAS_Y_SIZE));

				// remove unused consts in this function!

				marker->path_elements.push_back(std::make_shared<svg::svg_path_elements::C<double>>(weighted_sum(CENTER_X, CENTER_X),
				...
				marker->path_elements.push_back(std::make_shared<svg::svg_path_elements::C<double>>(weighted_sum(CENTER_X + COORD_DISTANCE_CORNER, CENTER_X),
				                                                                                    weighted_sum(CENTER_Y + COORD_DISTANCE_CORNER, CENTER_Y),
				                                                                                    weighted_sum(CENTER_X + HALF_CANVAS_X_SIZE, CENTER_X),
				                                                                                    weighted_sum(CENTER_Y, CENTER_Y),
				                                                                                    CENTER_X + HALF_CANVAS_X_SIZE,
				                                                                                    CENTER_Y));
				marker->path_elements.push_back(std::make_shared<svg::svg_path_elements::C<double>>(weighted_sum(CENTER_X + HALF_CANVAS_X_SIZE, CENTER_X),
				                                                                                    weighted_sum(CENTER_Y, CENTER_Y),
				                                                                                    weighted_sum(CENTER_X + COORD_DISTANCE_CORNER, CENTER_X),
				                                                                                    weighted_sum(CENTER_Y - COORD_DISTANCE_CORNER, CENTER_Y),
				                                                                                    CENTER_X + COORD_DISTANCE_CORNER,
		...
		class tobor_graphics {
			...
			template <class... T>
			inline static std::string draw_tobor_world(const tobor::v1_0::legacy_world<T...>& tw,
			...
			template <class... T>
			inline static std::string draw_tobor_world_with_cell_markers(const tobor::v1_0::legacy_world<T...>& tw, const std::vector<cell_id_type>& markers
			                                                             /*, coloring*/
			) {
