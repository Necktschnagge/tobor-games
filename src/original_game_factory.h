...
public:
	...
	using state_generator_type = tobor::v1_1::world_generator::
	   initial_state_generator<positions_of_pieces_type_interactive, 256, pieces_quantity_type::COUNT_TARGET_PIECES, pieces_quantity_type::COUNT_NON_TARGET_PIECES, 4>;
...
public:
	...
	[[nodiscard]] virtual AbstractGameController* create() const override {
		...
		if constexpr (pieces_quantity_type::COUNT_ALL_PIECES == 4) {
			...
			initial_color_permutation =
			   _product_generator.main().template obtain_permutation<std::vector<typename pieces_quantity_type::int_type>, pieces_quantity_type::COUNT_ALL_PIECES>(
			      initial_color_permutation);
		}

		return new DRWGameController<pieces_quantity_type>(world,
	...
	inline OriginalGameFactory& operator++() {
		++_product_generator;
		return *this;
	}
	...
	virtual std::size_t world_generator_group_size() const override { return board_generator_type::CYCLIC_GROUP_SIZE; }

	virtual std::size_t get_world_generator_counter() const override { return _product_generator.main().get_counter(); }

	virtual void set_world_generator_counter(std::size_t c) override { _product_generator.main().set_counter(c); }

	virtual std::size_t state_generator_group_size() const override { return state_generator_type::CYCLIC_GROUP_SIZE; }

	virtual std::size_t get_state_generator_counter() const override { return _product_generator.side().get_counter(); }

	virtual void set_state_generator_counter(std::size_t c) override { _product_generator.side().set_counter(c); }
	...
	virtual std::pair<std::string, std::size_t> svg_highlighted_targets() const override {
		...
		std::transform(raw_cell_id_vector.cbegin(), raw_cell_id_vector.cend(), std::back_inserter(comfort_cell_id_vector), [&](const auto& raw_cell_id) {
			return cell_id_type::create_by_id(raw_cell_id, world);
		});

		std::string svg_string = graphics::draw_source_board_with_cell_markers(world, comfort_cell_id_vector);
