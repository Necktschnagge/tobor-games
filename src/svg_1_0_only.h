...
namespace tobor {
	namespace v1_0 {
		namespace svg {
			...
			template <class... T>
			inline std::unique_ptr<fsl::i::stringable> draw_tobor_background(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss) {
				auto svg_background            = std::make_unique<latest::svggen::path>();
			...
			public:
				virtual std::unique_ptr<fsl::i::stringable> operator()(const world_type&                                 legacy_world,
				                                                       const drawing_style_sheet&                        dss,
				                                                       const tobor::v1_0::redundant_cell_id<world_type>& cell,
				                                                       const std::string&                                color) override {

					auto marker = std::make_unique<latest::svggen::path>();
					...
					marker->path_commands.push_back(std::make_shared<latest::svggen::pathc::M<double>>(CENTER_X, CENTER_Y + HALF_CANVAS_Y_SIZE));

					// remove unused consts in this function!

					marker->path_commands.push_back(std::make_shared<svg::svg_path_elements::C<double>>(weighted_sum(CENTER_X, CENTER_X),
					...
					marker->path_commands.push_back(std::make_shared<svg::svg_path_elements::C<double>>(weighted_sum(CENTER_X + COORD_DISTANCE_CORNER, CENTER_X),
					                                                                                    weighted_sum(CENTER_Y + COORD_DISTANCE_CORNER, CENTER_Y),
					                                                                                    weighted_sum(CENTER_X + HALF_CANVAS_X_SIZE, CENTER_X),
					                                                                                    weighted_sum(CENTER_Y, CENTER_Y),
					                                                                                    CENTER_X + HALF_CANVAS_X_SIZE,
					                                                                                    CENTER_Y));
					marker->path_commands.push_back(std::make_shared<svg::svg_path_elements::C<double>>(weighted_sum(CENTER_X + HALF_CANVAS_X_SIZE, CENTER_X),
					                                                                                    weighted_sum(CENTER_Y, CENTER_Y),
					                                                                                    weighted_sum(CENTER_X + COORD_DISTANCE_CORNER, CENTER_X),
					                                                                                    weighted_sum(CENTER_Y - COORD_DISTANCE_CORNER, CENTER_Y),
					                                                                                    CENTER_X + COORD_DISTANCE_CORNER,
					...
					marker->path_commands.push_back(std::make_shared<svg::svg_path_elements::Z>());
			...
			inline static std::string draw_tobor_world(const tobor::v1_0::legacy_world<T...>& tw,
				...
				/* ORDER REQUIREMENTS
				   ...
				   target is in front of blocked							// if by error, target is blocked, we still see target, we also indirectly see it is blocked by
			...
			template <class... T>
			inline std::unique_ptr<latest::svggen::path> get_vertical_grid_element(const tobor::v1_0::legacy_world<T...>& legacy_world,
			                                                                       const latest::svggen::style&           dss,
			                                                                       std::size_t                            cell_count_offset) {
				auto grid_element = std::make_unique<latest::svggen::path>();

				grid_element->fill()         = "dimgrey"; // !80 #0F54DA";
				grid_element->stroke()       = grid_element->fill();
				grid_element->stroke_width() = "0";

				auto start_at_upper_left = std::make_shared<latest::svggen::pathc::M<double>>(dss.LEFT_PADDING - dss.HALF_GRID_LINE_WIDTH + dss.CELL_WIDTH * cell_count_offset,
				                                                                              dss.TOP_PADDING - dss.HALF_GRID_LINE_WIDTH);
				auto go_to_right_bottom_left = std::make_shared<latest::svggen::pathc::l<double>>(dss.HALF_GRID_LINE_WIDTH * 2,
				                                                                                  0,
				                                                                                  0,
				                                                                                  dss.CELL_HEIGHT * legacy_world.get_vertical_size() +
				                                                                                     dss.HALF_GRID_LINE_WIDTH * 2,
				                                                                                  -dss.HALF_GRID_LINE_WIDTH * 2,
				                                                                                  0);
				auto go_back                 = std::make_shared<latest::svggen::pathc::Z>();
			...
			template <class... T>
			inline std::unique_ptr<latest::svggen::path> get_horizontal_grid_element(const tobor::v1_0::legacy_world<T...>& legacy_world,
			                                                                         const style&                           dss,
			                                                                         std::size_t                            cell_count_offset) {
				auto grid_element = std::make_unique<latest::svggen::path>();
				...
				auto start_at_upper_left    = std::make_shared<latest::svggen::pathc::M<double>>(dss.LEFT_PADDING - dss.HALF_GRID_LINE_WIDTH,
                                                                                          dss.TOP_PADDING - dss.HALF_GRID_LINE_WIDTH + dss.CELL_HEIGHT * cell_count_offset);
				auto go_to_bottom_right_top = std::make_shared<latest::svggen::pathc::l<double>>(0,
				                                                                                 dss.HALF_GRID_LINE_WIDTH * 2,
			...
			template <class... T>
			inline std::unique_ptr<fsl::i::stringable> draw_tobor_grid(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss) {
			...
			template <class... T>
			inline std::unique_ptr<latest::svggen::path>
			   get_horizontal_south_wall(const tobor::v1_0::legacy_world<T...>& legacy_world, const style& dss, std::size_t x, std::size_t y) {
				auto horizontal_wall_element = std::make_unique<latest::svggen::path>();
				...
				auto start_at_upper_left = std::make_shared<latest::svggen::pathc::M<double>>(dss.LEFT_PADDING + dss.CELL_WIDTH * x,
				                                                                              dss.TOP_PADDING - dss.HALF_WALL_LINE_WIDTH +
				                                                                                 dss.CELL_HEIGHT * (legacy_world.get_vertical_size() - y));

				auto left_semicircle = std::make_shared<latest::svggen::pathc::c<double>>(-dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
				                                                                          0,
				...
				auto go_right = std::make_shared<latest::svggen::pathc::l<double>>(dss.CELL_WIDTH, 0);

				auto right_semicircle = std::make_shared<latest::svggen::pathc::c<double>>(dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
				                                                                           0,
				                                                                           dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
				                                                                           -2 * dss.HALF_WALL_LINE_WIDTH,
				                                                                           0,
			...
			template <class... T>
			inline std::unique_ptr<fsl::i::stringable> draw_walls(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss) {
			...
			inline std::unique_ptr<latest::svggen::path>
			   get_vertical_west_wall(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss, std::size_t x, std::size_t y) {
				auto vertical_wall_element = std::make_unique<latest::svggen::path>();
				...
				auto start_at_lower_left = std::make_shared<latest::svggen::pathc::M<double>>(dss.LEFT_PADDING - dss.HALF_WALL_LINE_WIDTH + dss.CELL_WIDTH * x,
				                                                                              dss.TOP_PADDING + dss.CELL_HEIGHT * (legacy_world.get_vertical_size() - y));

				auto bottom_semicircle = std::make_shared<latest::svggen::pathc::c<double>>(0,
				                                                                            dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
				                                                                            2 * dss.HALF_WALL_LINE_WIDTH,
				                                                                            dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
				                                                                            2 * dss.HALF_WALL_LINE_WIDTH,
				...
				auto go_north = std::make_shared<latest::svggen::pathc::l<double>>(0, -dss.CELL_WIDTH);

				auto top_semicircle = std::make_shared<latest::svggen::pathc::c<double>>(0,
				                                                                         -dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
				                                                                         -2 * dss.HALF_WALL_LINE_WIDTH,
				                                                                         -dss.HALF_WALL_LINE_WIDTH * 2 * dss.WALL_CORNER_STRETCH,
				                                                                         -2 * dss.HALF_WALL_LINE_WIDTH,
			...
			inline std::unique_ptr<latest::svggen::path>
			   fill_whole_cell(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss, const cell_id_type& cell_id, const std::string& color) {
				auto whole_cell = std::make_unique<latest::svggen::path>();
				...
				auto start_at_north_west =
				   std::make_shared<latest::svggen::pathc::M<double>>(dss.LEFT_PADDING + dss.CELL_WIDTH * cell_id.get_x_coord(),
				                                                      dss.TOP_PADDING + dss.CELL_HEIGHT * (legacy_world.get_vertical_size() - cell_id.get_y_coord() - 1));

				auto go_east = std::make_shared<latest::svggen::pathc::l<double>>(dss.CELL_WIDTH, 0);

				auto go_south = std::make_shared<latest::svggen::pathc::l<double>>(0, dss.CELL_HEIGHT);

				auto go_west = std::make_shared<latest::svggen::pathc::l<double>>(-dss.CELL_WIDTH, 0);
			...
			template <class... T>
			inline std::unique_ptr<fsl::i::stringable> draw_blocked_cells(const tobor::v1_0::legacy_world<T...>& legacy_world, const drawing_style_sheet& dss) {
			...
			public:
				virtual std::unique_ptr<fsl::i::stringable> operator()(const world_type&                                 legacy_world,
					...
					auto duck = std::make_unique<latest::svggen::path>();
					...
					auto start_at_south_west = std::make_shared<latest::svggen::pathc::M<double>>(CELL_CORNER_SOUTH_WEST_x + dss.HORIZONTAL_PIECE_PADDING,
					                                                                              CELL_CORNER_SOUTH_WEST_y - dss.VERTICAL_PIECE_PADDING);

					auto go_bottom_east = std::make_shared<latest::svggen::pathc::l<double>>(dss.CELL_WIDTH - 2 * dss.HORIZONTAL_PIECE_PADDING, 0);

					auto go_bottom_right_corner_north = std::make_shared<latest::svggen::pathc::l<double>>(0, -FOOT_THICKNESS_RIGHT);

					auto go_back_neck_bottom_up = std::make_shared<latest::svggen::pathc::c<double>>(-0.5 * CANVAS_X_SIZE,
					                                                                                 0,
					                                                                                 -0.5 * CANVAS_X_SIZE,
					                                                                                 0,
					                                                                                 -0.4 * CANVAS_X_SIZE,
					...
					auto go_head = std::make_shared<latest::svggen::pathc::a<double>>(latest::svggen::pathc::a<double>::step(dss.PIECE_HEAD_X_RADIUS_FACTOR * MINIMUM_CANVAS_SIZE,
					...
					auto go_nort_west_for_beak = std::make_shared<latest::svggen::pathc::l<double>>(-0.33 * MINIMUM_CANVAS_SIZE, -0.15 * MINIMUM_CANVAS_SIZE);

					auto go_south_east_for_beak_down = std::make_shared<latest::svggen::pathc::l<double>>(0.32 * MINIMUM_CANVAS_SIZE, 0.25 * MINIMUM_CANVAS_SIZE);

					auto go_back = std::make_shared<latest::svggen::pathc::Z>();

					duck->path_commands = { start_at_south_west,         go_bottom_east, go_bottom_right_corner_north, go_back_neck_bottom_up, go_head, go_nort_west_for_beak,
						                     go_south_east_for_beak_down, go_back };

					auto outer_eye = std::make_unique<latest::svggen::path>();

					outer_eye->fill()         = "white";
					outer_eye->stroke()       = "black";
					outer_eye->stroke_width() = std::to_string(dss.PIECE_LINE_WIDTH);

					auto start_eye = std::make_shared<latest::svggen::pathc::M<double>>(CELL_CORNER_SOUTH_WEST_x + dss.HORIZONTAL_PIECE_PADDING + 0.66 * MINIMUM_CANVAS_SIZE,
					                                                                    CELL_CORNER_SOUTH_WEST_y - (dss.VERTICAL_PIECE_PADDING + 0.77 * MINIMUM_CANVAS_SIZE));

					outer_eye->path_commands.push_back(start_eye);

					outer_eye->path_commands.push_back(std::make_shared<latest::svggen::pathc::a<double>>(
					   latest::svggen::pathc::a<double>::step(0.04 * MINIMUM_CANVAS_SIZE, 0.03 * MINIMUM_CANVAS_SIZE, -10.0, true, false, 0.2, 0.2)));
