 ...
 * QA: discontinued
 * 
 * Note: This world has been deprecated because it uses the same integer type for size and cell ids
 *
 */
...
namespace tobor {
	namespace v1_0 {
		...
		class legacy_world {
			...
			legacy_world(const int_type& x_size, const int_type& y_size) : legacy_world() { resize(x_size, y_size); }
			...
			*/
			inline void block_center_cells(int_type x_blocked_size, int_type y_blocked_size) {
				...
				if ((x_size - x_blocked_size) % 2) throw division_by_2_error();
				if ((y_size - y_blocked_size) % 2) throw division_by_2_error();

				// check for non-blocked cells in every direction
				if (x_blocked_size >= x_size) throw blocked_center_error();
				if (y_blocked_size >= y_size) throw blocked_center_error();
			...
			inline constexpr int_type coordinates_to_cell_id(int_type x_coord, int_type y_coord) const noexcept { return x_size * y_coord + x_coord; }

			inline constexpr int_type coordinates_to_transposed_cell_id(int_type x_coord, int_type y_coord) const noexcept { return y_size * x_coord + y_coord; }

			inline constexpr std::pair<int_type, int_type> cell_id_to_coordinates(int_type id) const noexcept { return std::make_pair(id % x_size, id / x_size); }
			...
			inline constexpr int_type id_to_transposed_id(int_type id) const noexcept { return y_size * (id % x_size) + id / x_size; }

			inline constexpr int_type transposed_id_to_id(int_type transposed_id) const noexcept { return x_size * (transposed_id % y_size) + transposed_id / y_size; }

			/* wall accessors **************************************************************************************/

			inline wall& south_wall_by_transposed_id(int_type transposed_id) noexcept { return h_walls[transposed_id]; }

			inline const wall& south_wall_by_transposed_id(int_type transposed_id) const noexcept { return h_walls[transposed_id]; }

			inline wall& north_wall_by_transposed_id(int_type transposed_id) noexcept { return h_walls[transposed_id + 1]; }

			inline const wall& north_wall_by_transposed_id(int_type transposed_id) const noexcept { return h_walls[transposed_id + 1]; }

			inline wall& west_wall_by_id(int_type id) noexcept { return v_walls[id]; }

			inline const wall& west_wall_by_id(int_type id) const noexcept { return v_walls[id]; }

			inline wall& east_wall_by_id(int_type id) noexcept { return v_walls[id + 1]; }

			inline const wall& east_wall_by_id(int_type id) const noexcept { return v_walls[id + 1]; }
			 ...
			 */
			inline int_type count_cells() const noexcept { return x_size * y_size; }
			 ...
			 */
			inline int_type get_horizontal_size() const noexcept { return x_size; }

			/**
			 *	@brief Returns the board's number of cells in a column.
			 */
			inline int_type get_vertical_size() const noexcept { return y_size; }

			type turn_left_90() const { // only for quadratic
				if (x_size != y_size) { throw std::logic_error("Cannot turn for non-quadratic world."); }
