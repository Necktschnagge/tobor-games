...
+ "</svg>";
	...
	std::unique_ptr<svg_environment> body;
	std::string height;
	std::string width;

public:

	svg_environment(const std::string& height, const std::string& width, std::unique_ptr<svg_environment> body) :
		body(std::move(body)),
		height(height),
		width(width)
	{}
...
void test() {
	...
	auto svg_env = std::make_unique<svg_environment>(body);
